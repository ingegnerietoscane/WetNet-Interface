...
d3.gantt = function(id) {
	...
	var keyFunction = function(d) {
		return new Date(d.startDate) + d.taskName + new Date(d.endDate);
	};
	...
	function gantt(tasks) {
		...
		var svg = d3.select('#' + id).append("svg").attr("id", "svg-" + id).attr("class", "chart").attr("width", width + margin.left + margin.right).attr("height", height + margin.top + margin.bottom).append("g").attr(
				"class", "gantt-chart").attr("width", width + margin.left + margin.right).attr("height", height + margin.top + margin.bottom).attr("transform",
	...
	/**
	 * @param {string}
	 *            vale The value can be "fit" - the domain fits the data or "fixed" - fixed domain.
	 */
